...
export async function loadCobrancaData() {
  ...
  if (typeof window.renderCallDutyPopup === 'function') window.renderCallDutyPopup();
...
// --- VERSÃO LIMPA E FIXA DO POPUP (SEM MINIMIZAR) ---

export function renderCallDutyPopup() {
    // 1. Injeção de Estilos - Mantendo seu Azul #007bff e permitindo cliques
    if (!document.getElementById('call-duty-styles')) {
        const style = document.createElement('style');
        style.id = 'call-duty-styles';
        style.innerHTML = `
            .call-duty-popup {
                position: fixed;
                bottom: 25px;
            ...
            .call-duty-popup.minimized { width: 180px; padding: 10px 15px; cursor: pointer; }
            .call-duty-header {
                font-weight: 800; color: #2c3e50; font-size: 15px;
                margin-bottom: 12px; border-bottom: 2px solid #f1f1f1;
                padding-bottom: 8px; display: flex; align-items: center;
                justify-content: space-between; gap: 8px;
            }
            .call-duty-popup.minimized .call-duty-header { margin-bottom: 0; border-bottom: none; padding-bottom: 0; }
            .btn-min-popup {
            ...
            .duty-shift { margin-bottom: 10px; }
            .call-duty-popup.minimized .duty-shift { display: none; }
            .duty-label { display: block; font-size: 10px; color: #95a5a6; text-transform: uppercase; font-weight: bold; }
            .duty-name { display: block; font-size: 15px; color: #007bff; font-weight: 700; }
        `;
        document.head.appendChild(style);
    }

    // 2. Cronograma de Atendentes
    const schedule = {
        1: { morning: "Maria Júlia", afternoon: "Monica" },
        2: { morning: "Gilreania", afternoon: "Natalia" },
        3: { morning: "Fernanda", afternoon: "Dayse" },
        4: { morning: "Lorranye", afternoon: "Rozana" },
        5: { morning: "Gilreania", afternoon: "Monica" }
    };

    const today = new Date().getDay(); 
    const duty = schedule[today];
    const morningName = duty ? duty.morning : "Sem Escala";
    const afternoonName = duty ? duty.afternoon : "Sem Escala";

    // 3. Criação da Interface
    let popup = document.getElementById('call-duty-display');
    if (!popup) {
        popup = document.createElement('div');
        popup.id = 'call-duty-display';
        popup.className = 'call-duty-popup';
        if (localStorage.getItem('call-duty-minimized') === 'true') popup.classList.add('minimized');
        document.body.appendChild(popup);
    }

    const isMin = popup.classList.contains('minimized');

    popup.innerHTML = `
        <div class="call-duty-header">
            <div><i class="fas fa-phone"></i> ${isMin ? 'Ligações' : 'Ligações de Hoje'}</div>
            <button class="btn-min-popup" onclick="window.toggleDutyPopup(event)">
                <i class="fas ${isMin ? 'fa-expand-alt' : 'fa-compress-alt'}"></i>
            </button>
        </div>
        <div class="duty-shift">
            <span class="duty-label">Manhã</span>
            <span class="duty-name">${morningName}</span>
        </div>
        <div class="duty-shift" style="margin-bottom:0;">
            <span class="duty-label">Tarde</span>
            <span class="duty-name">${afternoonName}</span>
        </div>
    `;

    // 4. Lógica de visibilidade baseada na classe .hidden (Padrão do seu sistema)
    const tabCobranca = document.getElementById('tab-cobranca');
    const isAdmin = window.currentUserRole === 'admin'; // Verifica a permissão global
    
    // Só mostra se for ADMIN e se a aba de cobrança estiver visível
    if (isAdmin && tabCobranca && !tabCobranca.classList.contains('hidden') && tabCobranca.offsetParent !== null) {
...
// Expõe a função globalmente para ser chamada pelo app.js e botões
window.renderCallDutyPopup = renderCallDutyPopup;

window.toggleDutyPopup = function(event) {
    if (event) event.stopPropagation();
    const popup = document.getElementById('call-duty-display');
    if (!popup) return;
    const isMinimizing = !popup.classList.contains('minimized');
    popup.classList.toggle('minimized');
    localStorage.setItem('call-duty-minimized', isMinimizing);
    renderCallDutyPopup();
};

// Listener global para cliques em abas e botões
document.addEventListener('click', (e) => {
    if (e.target.closest('.tab-item') || e.target.closest('button') || e.target.closest('.nav-tab')) {
        setTimeout(renderCallDutyPopup, 200);
    }
});
